...
export default async function EditProductPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ saved?: string }> }) {
  ...
  return <main className="admin-home">{query.saved === "1" ? <AdminSaveToast /> : null}<p className="eyebrow">Catalogue</p><h1>Modifier {product.name}</h1><ProductForm initialValue={{ id: product.id, name: product.name, nameAr: product.nameAr ?? "", description: product.description, descriptionAr: product.descriptionAr ?? "", priceDh: product.priceDh, slug: product.slug, isVisible: product.isVisible, images: product.images.map(({ id: imageId, url, alt, color, position }) => ({ id: imageId, url, alt, color, position })), variants: product.variants.map(({ id: variantId, sku, size, color, stock, _count }) => ({ id: variantId, sku, size, color, stock, historical: _count.orderItems > 0 })) }} /></main>;
}
