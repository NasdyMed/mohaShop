...
export default async function CatalogPage() {
  ...
  return <main>
    ...
    <section className="catalog shell" aria-labelledby="collection-title"><div className="section-heading"><h2 id="collection-title">Nos modèles</h2><span>{products.length} {products.length > 1 ? "modèles" : "modèle"}</span></div>{products.length > 0 ? <div className="product-grid">{products.map((product) => <ProductCard key={product.id} product={product} />)}</div> : <div className="empty-state"><p className="eyebrow">La collection se prépare</p><h2>De nouvelles bottes arrivent bientôt.</h2><p>Revenez dans quelques jours pour découvrir nos prochains modèles.</p></div>}</section>
  </main>;
}
