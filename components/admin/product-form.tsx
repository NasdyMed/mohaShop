...
export function ProductForm({ initialValue }: { initialValue?: Value }) {
  ...
  return <form className="product-form" onSubmit={submit} aria-busy={locked}>
    ...
    <label>Nom<input disabled={locked} required minLength={2} maxLength={120} value={value.name} onChange={(event) => setValue({ ...value, name: event.target.value })}/></label><FieldError errors={errors.name}/>
    <label>Slug<input disabled={locked} required pattern="[a-z0-9]+(?:-[a-z0-9]+)*" maxLength={120} value={value.slug} onChange={(event) => setValue({ ...value, slug: event.target.value })}/></label><FieldError errors={errors.slug}/>
    <label>Prix (DH)<input disabled={locked} required type="number" min={1} max={1_000_000} value={value.priceDh} onChange={(event) => setValue({ ...value, priceDh: Number(event.target.value) })}/></label><FieldError errors={errors.priceDh}/>
    <label>Description<textarea disabled={locked} required minLength={20} maxLength={3000} rows={8} value={value.description} onChange={(event) => setValue({ ...value, description: event.target.value })}/></label><FieldError errors={errors.description}/>
    ...
    <section><h2>Images</h2><FieldError errors={errors.images}/>
      ...
      {value.images.map((image, index) => <div className="admin-image-row" key={image.id ?? image.url}>
        ...
        <label>Texte alternatif<input disabled={locked} required minLength={2} maxLength={160} value={image.alt} onChange={(event) => setValue({ ...value, images: value.images.map((item, current) => current === index ? { ...item, alt: event.target.value } : item) })}/></label><FieldError errors={errors[`images.${index}.alt`]}/>
