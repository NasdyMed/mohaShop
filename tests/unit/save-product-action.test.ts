...
describe("saveProductAction", () => {
  ...
  it("sérialise les erreurs imbriquées avec leur chemin complet", async () => {
    ...
    const result = await saveProductAction({ ...input, priceDh: 1.5, description: "court", isVisible: true, images: [{ url: "https://images.unsplash.com/a.webp", alt: "", color: "Noir", position: 0 }], variants: [{ sku: "OK", size: "38", color: "Noir", stock: -1 }] });
  ...
  it("retourne une erreur de couleur explicite", async () => { mocks.requireAdmin.mockResolvedValue({}); mocks.saveProduct.mockRejectedValue(new ProductMutationError("INVALID_COLOR")); await expect(saveProductAction(input)).resolves.toMatchObject({ ok: false, code: "INVALID_COLOR", fieldErrors: { variants: [expect.any(String)] } }); });
